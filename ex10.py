def PrintingCosts(line):
    ...
    list = a.split('        ')
    symbol_dict = {}
    for i in list:  # цикл, перебирающий все символы и создающий словарь символ-ключ, значение - цифры
        if '\n' not in i:
            dict_key = i[0]
            dict_value = i[i.rfind(' ') + 1:]
        else:
            dict_key = i[0]
            dict_value = i[i.rfind(' ') + 1:-1]
        symbol_dict.update({dict_key: int(dict_value)})
    for i in line:  # цикл, считающий тонер, связан со словарём symbol_dict
        if i in symbol_dict:
            paint += symbol_dict[i]
        else:
            paint += 23
